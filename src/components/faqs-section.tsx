	...
	return (
		<div className="mx-auto w-full max-w-4xl space-y-10 px-4 py-8 md:px-10 md:py-10 xl:max-w-5xl">
			<div className="max-w-xl space-y-3">
				<h2 className="text-2xl font-semibold tracking-tight md:text-3xl">{t.faq}</h2>
				<p className="text-sm leading-relaxed text-muted-foreground">
					{t.faqDescription}
				</p>
			</div>
			<Accordion className="rounded-xl border border-border/60 bg-card/80 px-0 shadow-none backdrop-blur-sm" collapsible type="single">
				{questions.map((item) => (
					<AccordionItem key={item.id} value={item.id} className="border-border/50 px-4">
